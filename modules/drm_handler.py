...
from topic_handler import get_or_create_forum_topic, extract_autotopic_name, send_document_with_fallback, send_video_with_fallback, send_photo_with_fallback
...
YTDLP_BASE = (
    ...
    "--no-warnings "
    "--socket-timeout 30 "
    "--throttled-rate 100K "     # re-fetch if server throttles below 100K
    "--extractor-retries 5 "
    "--hls-use-mpegts "
    "--downloader-args 'aria2c:{ARIA2_OPTS}' "  # injected below
)

def _ytdlp_cmd(ytf: str, url: str, output: str, extra: str = "", cookies: str = None) -> str:
    """Build an optimised yt-dlp command string."""
    aria_arg = f"--downloader aria2c --downloader-args \"aria2c:{ARIA2C_OPTS}\""
    ck = f'--cookies "{cookies}"' if cookies and os.path.exists(cookies) else ""
    return (
        f'yt-dlp {aria_arg} '
...
# ---------------------------------------------------------
# YOUTUBE DOWNLOAD HANDLER (TURBO — aria2c + 16 threads)
# ---------------------------------------------------------
async def download_youtube(url, ytf, name, cookies=None):
    output_file = f"{name}.mp4"
    cmd = _ytdlp_cmd(ytf, url, output_file, cookies=cookies)
